...
function ResponsiveContainer({ width = '100%', height = 240, style, children, ...rest }) {
  ...
  return React.createElement('div', Object.assign({}, rest, { style: merged }), children);
}
...
function BarChart({ data = [], children, style, barGap = 16 }) {
  ...
  return React.createElement('div', { style: containerStyle }, barElement);
}
BarChart.__isRechartsBarChart = true;

function Bar({ dataKey, fill = 'currentColor', radius = 0, data = [], xKey }) {
  ...
  return items.map((item, idx) => {
    ...
    return React.createElement(
      ...
      React.createElement(
        'div',
        { style: { flex: 1, display: 'flex', alignItems: 'flex-end', width: '100%' } },
      ...
      React.createElement(
        'div',
        { style: { fontSize: '12px', color: 'rgba(15,23,42,0.72)', textAlign: 'center' } },
        item.label,
      ),
    );
  });
...
function PieChart({ children, style }) {
  const merged = Object.assign(
    {
      width: '100%',
      height: '100%',
  ...
  return React.createElement('div', { style: merged }, children);
}
PieChart.__isRechartsPieChart = true;

function Pie({ data = [], dataKey, innerRadius = 0, outerRadius = 120, children }) {
  const fills = [];
